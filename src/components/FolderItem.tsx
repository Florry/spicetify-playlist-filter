...
const FolderItem = ({ searchTerm, folder, indentation = 0, deadEnd, recursiveOpen }: Props) => {
    const { sortOption, sortOptionWithoutFiltering, isSortingWithoutFiltering, currentlyPlayingUri, openLibraryFolders, draggingSourceUri, draggingSourceName, onDraggingDropped } = useFilterContext();
    const [folderIsOpen, setFolderIsOpen] = React.useState(recursiveOpen || isSortingWithoutFiltering && openLibraryFolders.includes(folder.uri));
    ...
    const createPlaylist = async () => {
        ...
        try {
            ...
            Spicetify.showNotification("Playlist created");
    ...
    const sortedItems = useMemo(() => {
        ...
        return items
            .sort((a, b) => sortItems(a, b, searchTerm, isSortingWithoutFiltering ? sortOptionWithoutFiltering : sortOption))
